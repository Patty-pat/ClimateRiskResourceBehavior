...
def _create_ROUND(df_combined):
    # Creating new columns
    df_combined["ROUND"] = df_combined["subsessionround_number_1"]
    return df_combined


def _create_relative_player_harvest_in_baseline(df_combined):
    df_combined["relative_playertake_1"] = df_combined["playertake_1"] / 7
    return df_combined


def _create_relative_group_harvest_in_baseline(df_combined):
    df_combined["relative_grouptake_1"] = df_combined["grouptotal_group_take_1"] / 21
    return df_combined


def _create_starting_number_of_trees_in_each_round(df_combined):
    # Create start trees
...
def _create_relative_starting_trees(df_combined):
    df_combined["rel_starttrees_1"] = df_combined["start_trees_1"] / 100
    return df_combined


def _create_other_group_members_harvest(df_combined):
...
def _rename_variables_baseline(df_combined):
    rename_dict = {
...
def clean_baseline_data(df_main):
    df_combined = _keep_and_append_to_long(df_main)
    df_combined = _create_ROUND(df_combined)
    df_combined = _create_relative_player_harvest_in_baseline(df_combined)
    df_combined = _create_relative_group_harvest_in_baseline(df_combined)
    df_combined = _create_starting_number_of_trees_in_each_round(df_combined)
    df_combined = _create_relative_starting_trees(df_combined)
    df_combined = _create_other_group_members_harvest(df_combined)
    df_combined = _rename_variables_baseline(df_combined)
