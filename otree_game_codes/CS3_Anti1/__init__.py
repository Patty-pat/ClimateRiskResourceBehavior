...
class Group(BaseGroup):
    ...
    def tree_count_regrow(group):
        ...
        if p1.participant.current_trees_after_take < 91:
            p1.participant.current_trees = (
                p1.participant.current_trees_after_take
                + p1.participant.current_trees_after_take / 10
            )
...
def round_which(label):
    return models.IntegerField(
        choices=[[0, "Runde 1"], [1, "Runde 5"], [2, "Kann man pauschal nicht sagen."]],
...
def my_field(label):
    return models.IntegerField(
        choices=[[1, "Ja"], [0, "Nein"], [2, "Kann man pauschal nicht sagen."]],
        widget=widgets.RadioSelectHorizontal,
        label=label,
    )
...
class Player(BasePlayer):
    take = models.IntegerField(
        min=C.MIN_TAKE,
        max=C.MAX_TAKE,
        label="Wie viele Bäume möchten Sie entnehmen?",
    )
    ...
    compr1 = round_which(
        label="1. Falls ein Waldbrand eintritt, in welcher Runde tritt er ein?",
    )
    compr2 = my_field(
        "2. Falls ein Waldbrand eintritt, wird der Schock zu Beginn der Runde eintreten bevor der Wald nachwachsen konnte? ",
    )
    compr3 = percent(
        label="3. Falls ein Waldbrand eintritt, um wie viel Prozent wird die Anzahl der verbliebenen Bäume reduziert?",
    )
    compr4 = my_field(
        "4. Wirkt sich der Waldbrand auf die Anzahl Ihrer Punkte aus, die Sie am Ende erhalten?",
    )
    ...
    def set_balance(player):
        ...
        player.balance = (
            ...
            + total_player_income
            - total_player_costs
            + C.MULTIPLIER_TAKE * total_player_take
        )
    ...
    if subsession.round_number == 1:
        ...
        for g in subsession.get_groups():
            ...
            for p in players:
                ...
                print(
                    "Group "
                    + str(g.id_in_subsession)
                    + ", Player "
                    + str(p.id_in_group)
                    + ": "
                    + str(p.participant.high_probability),
                )
    else:
        for g in subsession.get_groups():
        ...
        for g in subsession.get_groups():
            ...
            for p in g.get_players():
                ...
                print(
                    ...
                    + str(p.id_in_group)
                    + ": "
                    + str(p.participant.is_shock_group),
                )
...
class CS3Example(Page):
    ...
    def js_vars(player: Player):
        return {"INITIAL_TREES": C.INITIAL_TREES}
...
class P05ComprehensionCheckA(Page):
    ...
    def error_message(player: Player, values):
        solutions = {"compr1": 1, "compr2": 0, "compr3": 1, "compr4": 1}
        names = {
            "compr1": "Frage 1",
            "compr2": "Frage 2",
            "compr3": "Frage 3",
        ...
        errors = {
            name: "Falsch:" for name in solutions if values[name] != solutions[name]
        }
        if errors:
            player.num_failed_attempts += 1
            error_names = [names[name] for name in errors]
...
class Forest2_regrowth(Page):
    ...
    def vars_for_template(player: Player):
        group = player.group
        p1 = group.get_player_by_id(1)
        group.current_trees_after_take = round(
            p1.participant.current_trees_after_take,
            2,
        )
        group.current_trees = round(p1.participant.current_trees, 2)
        ...
        return {
            "current_trees_before": current_trees_before,
    ...
    def js_vars(player):
        ...
        return {
            ...
            "current_trees_after_take": math.floor(
                p1.participant.current_trees_after_take,
            ),
            "current_trees": math.floor(p1.participant.current_trees),
...
class ResultsWaitPage(WaitPage):
    ...
    def after_all_players_arrive(group: Group):
        for p in group.get_players():
            ...
            p.participant.take_history.append(
                p.player_history_take,
            )  ##### I removed an if statement - it's obsolete because I'm referring to the apps by name in the end
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        ...
        p1 = group.get_player_by_id(1)
        if group.is_shock_group is True:
            player.stage_points = round(
                player.balance
                + p1.participant.current_trees
                * C.MULTIPLIER_SHOCK
                * C.MULTIPLIER_GROUP
            ...
            return {
                ...
                "stage_value_aftershock": round(
                    group.current_trees * C.MULTIPLIER_SHOCK * C.MULTIPLIER_GROUP,
                    2,
                ),
                "trees_stage_value": round(
                    (group.current_trees * C.MULTIPLIER_GROUP) / 2,
                    2,
                ),
                "trees_stage_value_aftershock": round(
                    (group.current_trees * C.MULTIPLIER_SHOCK * C.MULTIPLIER_GROUP) / 2,
                    2,
                ),
                "stage_share": round(
                    group.current_trees * C.MULTIPLIER_GROUP / C.PLAYERS_PER_GROUP,
                    2,
                ),
                "stage_share_aftershock": round(
                    group.current_trees
                    * C.MULTIPLIER_SHOCK
                    * C.MULTIPLIER_GROUP
                    / C.PLAYERS_PER_GROUP,
